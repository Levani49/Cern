...
import { eventFileIsValid } from "#/utils/eventFileIsValid";
      ...
      if (file && file.name.endsWith(".xml")) {
        ...
        reader.onload = (e: ProgressEvent<FileReader>): void => {
          if (e.target) {
            ...
            const parsedXmlData = EventHandler.parseXmlAsJSON(xmlContent);
  ...
  return (
    <>
      <div className="flex w-full items-center justify-between text-accent3">
        ...
        >
          <Icons.ArrowUpTrayIcon className="icon text-accent3" />
        </button>
        <div className="flex items-center gap-2">
          ...
          >
            <Icons.ChevronLeftIcon className="icon text-accent3" />
          </button>
          ...
          >
            <Icons.ChevronRightIcon className="icon text-accent3" />
          </button>
        </div>
        <button
        ...
        >
          <Icons.FolderIcon className="icon text-accent3" />
        </button>
      </div>
      <div
