...
import CameraViews from "#/models/Camera";
...
import eventsEmitter from "#/utils/eventEmitter";
import { startDroneMode, stopDroneMode } from "#/utils/handleDrone";
...
const initialState: CameraSettings = {
  defaultPosition: defaultPosition as Coordinates,
  currentState: "idle",
  droneType: "idle",
  camera: null,
...
export const cameraSlice = createSlice({
  ...
  reducers: {
    rehydrate: (state, action) => {
      const newState = action.payload.camera as CameraSettings;
    ...
    triggerCameraEffect: (
      state,
      action: PayloadAction<CameraSettings["triggerCameraEffect"]>
    ) => {
...
export const selectCameraEffect = (
  state: RootState
): CameraSettings["triggerCameraEffect"] => state.camera.triggerCameraEffect;
